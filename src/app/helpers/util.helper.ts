...
export class Util {
  ...
  static async openRemoveDialog(config?: any) {
    ...
    // nconfig["data"] =
    //   !config || !config.data
    //     ? { title: "Warning", body: "Are you sure you want to delete this?" }
    //     : config.data;
  ...
  static async post(url, data) {
    ...
    [err, res] = await this.to(
      this.http.post(url, data, { headers: headers }).toPromise(),
      false
    );
  ...
  static async put(url, data) {
    ...
    let err, res;
    [err, res] = await this.to(
      this.http.put(url, data, { headers: headers }).toPromise(),
      false
    );
    return this.responder(err, res);
  }
