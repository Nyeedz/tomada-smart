...
export class Model {
  ...
  reload() {
    //updates instance storage from browser data
  ...
  storageDifference() {
    ...
    let storage_differences = _.pick(
      storage,
      (value: any, key: any, object: any) => {
  ...
  instanceDifference() {
    ...
    let instance_differences = _.pick(
      instance,
      (value: any, key: any, object: any) => {
        return diff.includes(key);
      }
  ...
  static instantiateObject(obj_data: any, single?: boolean) {
    //this gets it so the object has the same reference and thus data in components
    let obj: any;
    ...
    obj = this._instances.filter(
      (instance: any) => instance[primary_key] === obj_data[primary_key]
    )[0];
  ...
  static create(data: any, single?: boolean) {
    ...
    let check = old_data.filter(
      (m: any) => m[primary_key] === instance[primary_key]
    )[0];
    if (check) {
      throw Error(
        `Duplicate key in ${this.getModelName()} model, key: ${primary_key} value: ${
    ...
    if (!instance[primary_key] || instance[primary_key] == "") {
      throw Error(
        `No key given in ${this.getModelName()} model, key: ${primary_key} value: ${
          instance[primary_key]
        }`
      );
    }
  ...
  static removeInstance(search: object) {
    //Removes instance from the app storage
  ...
  static removeStorage(search: object) {
    //Removes instance from browser storage
    let all_data = this.getAllData();
    let new_data = all_data.filter((data: object) => {
      return !_.isMatch(data, search);
    });
  ...
  static updateOne(search: object, new_data: any, single?: boolean) {
    ...
    let instance: any = all_data.filter((data: any) => {
      return _.isMatch(data, search);
    })[0];
    if (!instance) {
      return null;
  ...
  static findOneAndUpdate(search: object, data?: any, options?: any) {
    ...
    let all_data = this.getAllData();
    let instance = all_data.filter((data: object) => {
      return _.isMatch(data, search);
    })[0];
  ...
  static on(events: any, listener?: Function) {
    if (typeof events === "string") {
      ...
      return () => {
        this._events[events] = this._events[events].filter(
          (l: any) => l !== listener
        );
      };
    } else {
  ...
  static emit(events: any, data?: any) {
    if (typeof events === "string") {
      ...
      if (event_listeners)
        event_listeners.forEach((listener: any) => listener(data));
    } else {
      for (let i in events) {
        let kind = events[i];
